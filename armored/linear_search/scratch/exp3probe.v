...
(* The epilogue pins the state's R30 to the caller's return address [ret0] so
   ...
   c_call-ret continuation.  [linear_search_nf_spec_ep ret0] is like
   [linear_search_nf_spec] but with R30 := ret0 (not-found); likewise the
   found variant. *)
Definition linear_search_nf_spec_ep (ret0 : bv 64) : iProp Σ :=
  ∃ (base len tgt : bv 64) (data : list (bv 64)) (i' tmp : bv 64),
  reg_col sys_regs ∗
  reg_col CNVZ_regs ∗
  "R0" ↦ᵣ RVal_Bits base ∗
  "R1" ↦ᵣ RVal_Bits len ∗
...
Definition linear_search_f_spec_ep (ret0 : bv 64) : iProp Σ :=
  ∃ (base len tgt : bv 64) (data : list (bv 64)) (i' tmp : bv 64),
  reg_col sys_regs ∗
  reg_col CNVZ_regs ∗
  "R0" ↦ᵣ RVal_Bits base ∗
...
(* [linear_search_loop_spec] is the loop invariant at address 0x4.  The
   ...
   exactly the register value the machine state already carries.  This lets
   the hypothesis-free top level derive the exit `instr_pre`s from the
   actual caller continuation at [bv_unsigned ret] instead of keeping them
   as hypotheses. *)
...
Abort.
  ...
  - assert (Hnextb : Z.to_nat (bv_unsigned (bv_extract 0 64 (bv_zero_extend 128 i) + 1)) = S (Z.to_nat (bv_unsigned i))) by bv_solve.
...
Proof.
  ...
  all: try bv_simplify_arith select (bv_extract _ _ _ = _).
  Unshelve.
  all: try (rewrite H0n in Htoo).
  Unshelve. all: try (simpl in Htoo; lia).
  Unshelve.
  all: try (iPureIntro; assumption).
  Unshelve.
  all: try (iApply instr_pre_intro_Some).
  all: try (iFrame).
  Unshelve.
  iIntros "Ha20".
  iExists ().
  iSplitR "Ha20".
  - iApply (instr_pre_intro_Some _ _ a20 _).
    + iAssumption.
    + iIntros "Hnf HPC".
      iDestruct "Hnf" as (base len tgt data0 i' tmp)
        "(Hsys & Hcnv & HR0 & HR1 & HR2 & HR3 & HR4 & HR30 &
          Hmem & %Heq_i & %Hlen_eq & %Hnmis)".
      liARun.
      (* ================================================================= *)
      (* WALL 1 (a20 branch, HAS the c_call continuation):                  *)
      (* liARun ran mvn+ret, found instr_pre (bv_unsigned ret)             *)
      (* (the caller continuation, return address MATCHES the pinned ret),  *)
      (* and stopped at the c_call-return obligation:                       *)
      (*     Hmem : bv_unsigned base ↦ₘ* data0                              *)
      (*     --------------------------------------∗                        *)
      (*     bv_unsigned b0  ↦ₘ* data ∗ ⌜nf-pure⌝ ∗ True                    *)
      (* iFrame cannot frame b0↦data: base/data0 are RIGID (iDestruct of    *)
      (* the exit premise); the wp needs EVAR witnesses (as when an exit is *)
      (* intro'd as a continuation) but the ∃ cannot be destructed lazily.  *)
      (* ================================================================= *)
      Show.
Abort.

(* The sibling branch (a28 / f-exit) is left out of the live proof but was
   exercised: with iSplitL "Ha20" (continuation given to THIS a20 branch
   instead), the a20 branch stops at the smallest exact residual
     find_in_context (FindInstrKind (bv_unsigned ret) true)
   with NO continuation candidate in scope (context only: Ha20, Hcnv,
   HR1..HR4, Hmem, R0=UINT64_MAX, sys_regs, R30) -- one affine caller
   continuation (instr_pre is __not__ Persistent), two exits to satisfy.
   See scratch/exp3_residual_report.md for the full evidence. *)

Lemma linear_search_debug stack_size :
  0 ≤ stack_size →
  instr 0x0000000010300000 (Some a0) -∗
  instr 0x0000000010300004 (Some a4) -∗
  instr 0x0000000010300008 (Some a8) -∗
  ...
  instr 0x0000000010300028 (Some a28) -∗
  instr 0x000000001030002c (Some a2c) -∗
  instr_pre 271581216 linear_search_nf_spec -∗
  instr_pre 271581224 linear_search_f_spec -∗
  □ instr_pre 0x0000000010300004 linear_search_loop_spec -∗
  instr_body 0x0000000010300000 (linear_search_spec stack_size).
Proof.
  move => ?. iStartProof.
  ...
  Unshelve. all: try (simpl in Htoo; lia).
  (* NOTE: left open by design.  The authoritative hypothesis-based baseline
     (exits as `instr_pre 0x20 linear_search_nf_spec /
     instr_pre 0x28 linear_search_f_spec`) is `../linear_search_proof.v`
     (`linear_search', lines 331-349), which Qed's upstream.  This scratch
     copy is kept only as a reminder that hypothesis-free composition breaks
     exactly where the exits must be rebuilt. *)
Abort.
